  ...
  <div class="player" v-show="playlist.length > 0">
    ...
    >
      <div class="normal-player" v-show="fullScreen">
        ...
        <div
          class="middle"
          @touchstart.prevent="middleTouchStarst"
          ...
          <div class="operators">
            ...
            <div class="icon i-right">
              <i
                class="icon"
                :class="favoriteIcon"
                @click="toggoleFavorite(currentSong)"
              ></i>
            </div>
          </div>
        </div>
...
export default {
  ...
  data() {
    return {
      songReady: false, // 切换歌曲
      currentTime: 0,
      radius: 32,
      currentLyric: null,
      currentLineNum: 0, // 歌词的当前下标
      currentShow: 'cd',
      playingLyric: ''
    };
  },
  computed: {
    ...
    ...mapGetters([
      'fullScreen', // 是否展开
      'playing', // 这里的playing映射到getters里面的playing，getters里面的playing是对state里面playing的包装
      'currentIndex'
    ])
  },
  created() {
  ...
  methods: {
    ...
    middleTouchEnd() {
      ...
      if (this.currentShow === 'cd') { // 从左向右滑
        ...
        if (this.touch.percent < 0.9) { // 从右向左滑
          offsetWidth = 0;
          opacity = 1;
          this.currentShow = 'cd';
        } else {
  ...
  watch: {
    currentSong(newSong, oldSong) {
      ...
      //   this.$refs.audio.play();
      //   this.getLyric();
      // }); 不用this.$nextTick是考虑到在手机端操作时在后台运行js不执行 但歌曲播放问题 造成songReady不能置为true, 歌曲不能切换
    },
    playing(newPlaying) {
